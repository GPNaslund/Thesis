...
extension OpenMHealthBodyTemperatureConverter on HKBodyTemperature {
  List<OpenMHealthBodyTemperature> toOpenMHealthBodyTemperature() {
    ...
    if (data.metadata != null) {
      if (data.metadata!.containsKey("HKMetadataKeyBodyTemperatureSensorLocation")) {
        var tempLocation = data.metadata!["HKMetadataKeyBodyTemperatureSensorLocation"];
        ...
        log("Expected value of HKMetadataKeyBodyTemperatureSensor to be a number");
      }
    }

    var tempUnit = TemperatureUnit.C;
    var unitValue = TemperatureUnitValue(value: data.quantity.doubleValue, unit: tempUnit);
