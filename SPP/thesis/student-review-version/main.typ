...
This thesis is a 15 HEC Bachelor's thesis in Software Engineering. It explores the domain of mobile application development, wearable health technology, and data normalization. As healthcare becomes increasingly more digital, wearable devices like smartwatches and fitness trackers are becoming progressively established as tools for collecting physiological and behavioral data.

The widespread adoption and acceptance of these devices offers insightful possibilities for health monitoring, particularly for conditions like migraine that stand to gain from such monitoring. However, the current landscape is flawed because data formats and APIs are implemented differently among various providers. This divergence makes it difficult to integrate health data across platforms or use it effectively in research and machine learning. @impact-wearable-technologies

This thesis investigates this challenge and contributes by exploring how a mobile software framework solution might help unify and normalize health data from various sources. The objective is to simplify data handling for researchers and developers while also aiming to enabling more effective use of health data in fields such as machine learning.
...
RQ1. How can wearable health data from different platforms be effectively normalized into a unified format, thereby establishing a foundational step towards suitability for broader health applications, including specific aspects of migraine-focused machine learning that utilize physiological data such as HR and HRV?
...
The primary target group is software developers and researchers working within the field of digital health, particularly those focusing on mobile application developments and wearable data integration. A challenge these stakeholders potentially face is working around the fragmented health data formats when developing solutions or preparing datasets for machine learning solutions.
...
Upon completion of the data collection framework, its implemented functionality was subjected to a series of tests to demonstrate its capability in addressing the defined problem. These evaluations included functional, integration, and system testing to verify the core features and overall system performance. Subsequently, a collaborative assessment was conducted with the stakeholders. The framework's achieved functionality and performance were compared against their initially defined goals and requirements to determine how well the objectives were met. To further evaluate the artifact's practical relevance and usability from a developer perspective, a final semi-structured interview was conducted with Neurawave focusing on the plugin's features and potential integration into their workflow (see Appendix A).
...
To evaluate the developed solution, we will use experimentation as the primary method of validating functionality. This approach's iterative feedback suited our development process. To clarify the assessment of 'effectiveness' for RQ1 (normalization) and RQ3 (Open mHealth standard), our evaluation focused on:
For RQ1: the plugin's consistent extraction of HR/HRV data from diverse platforms and its accurate conversion into a unified Open mHealth format, including necessary extensions.
For RQ3: the Open mHealth standard's ability (with adaptations) to represent this physiological data without critical information loss while supporting unification.
According to Basili et al. [19], experimentation is an iterative process of hypothesizing and testing. In our case, this involves defining (or redefining) software requirements, implementing them (the hypothesis), and verifying whether those requirements are fulfilled (the test).

Additionally, we will perform validation to ensure that the plugin's software requirements align with the original stakeholder expectations @the-role-of-software-verification. As an alternative to experimentation, we considered survey-based research to gather stakeholder opinions on desired functionality. However, we ultimately decided against this, primarily because of our limited timeframe. We also believed that the iterative loop supported by experimentation would be hindered by the time required to create, distribute, and analyze a survey. In our context, experimentation offers more immediate feedback and supports rapid iteration, which we viewed as essential for effective development.
...
== 2.4 Wearables and data stores

We decided that the framework should target native health data stores because they provide a unified API for extracting health data that is not affected by third-party wearables. Since Android holds 71.9% of the global market share and iOS 27.68% @mobile-os-market, Google Health Connect and Apple HealthKit are the native data stores we primarily target. During the validation of the framework, we will perform experiments using Apple Watch, Google Fitbit, and Garmin smartwatches as providers of health data to the native data stores. The selection of wearables was determined by available resources, including hardware (Apple Watch, Fitbit Sense 2, Garmin Venu 2 ) kindly provided by the project stakeholder Neurawave , and the aim for reasonable coverage of common wearables.
...
#table(
  ...
  [], [*Health 12.0.1, Flutter package @health-package*], [Enables reading and writing health data to and from Apple health and Google Health Connect.], [ - Lacks support for several providers (Fitbit, Garmin, and others) \
  - No support for Open mHealth data format.],
  /* REACT NATIVE HEALTH */
  [], [*React Native Health, React native package @react-native-health*], [Package for interacting with Apple HealthKit for iOS.], [ - Lacks support for several providers (e.g., Google Health Connect). \
  - No support for Open mHealth.],
  /* REACT NATIVE HEALTH CONNECT */
  [], [*React native health connect, React native package @react-native-health-connect*], [Package for interacting with Health Connect for Android.], [- Lacks support for several providers (e.g Apple HealthKit on iOS) \
  - No support for Open mHealth.],
  /* SHIMMER */
...
The current state of e-health data standards is outlined below:
...
One requirement for this framework was that it must be cross-platform and mobile-native. Several development frameworks support this, including React Native @react-native, Flutter @flutter, LynxJS @lynx-js and Kotlin Multiplatform @kotlin-multi-platform.
...
No additional third-party plugins was used beyond what is required to interface with HealthKit and Health Connect.
...
The plugin was demonstrated through a dedicated test application built in Flutter. This application implemented the full data flow, including permission requests and data extraction. To evaluate data retrieval completeness, the number of datapoints extracted by the plugin was compared against a baseline. This baseline was established by querying the respective native health platform's user interface (e.g., Apple Health app, Google Health Connect interface) for the same data types over the identical time periods immediately prior to plugin extraction, providing an independent count of available records. In addition, the application included automated tests that validated the correctness of the data transformation and adherence to the Open mHealth schema. The test application was evaluated using real devices, such as the Apple Watch, Fitbit, and Garmin Venu, to simulate real-world usage and ensure compatibility across a range of wearables.
...
#table(
  ...
  [TEST-GN01], [Garmin Venu 2], [Samsung Galaxy S22], [Android 14], [6h], [HR],
...
This section details the data retrieval completeness of the wearable_health plugin. The completeness was assessed by comparing the number of datapoints extracted by the plugin against the total number of available datapoints within the native data stores for the specified test duration.
...
#table(
  ...
  align: horizon,
  table.header(
    [*Id*], [*Amount of datapoints*], [*Extraction time*], [*Conversion time*], [*Total time*],
  ),
  [TEST-GN02],  [7206], [1286ms], [3ms], [1289ms],
  [TEST-GN03], [51], [N/A], [N/A], [N/A],
  [TEST-GN01], [1406], [133ms], [\<1\*], [133ms],
  [TEST-DP01],  [589], [239ms], [\<1\*], [239ms],
  [TEST-DP02], [20], [N/A], [N/A], [N/A],
)
_Execution and conversion times for the real-time data tests (TEST-GN03, TEST-DP02) are marked as 'N/A'. These experiments were primarily designed to assess the plugin's stability and capability for continuous data querying and handling from native data stores, rather than to measure the specific end-to-end latency of individual datapoints from wearable capture to full normalization by the plugin in a true streaming architecture. While the batch conversion times (Table 5.5) indicate high efficiency of the normalization algorithms themselves, quantifying overall real-time system latency for RQ2 would necessitate a different experimental design focused on per-update processing times, which was beyond the scope of these specific stability-focused tests. The focus was on confirming the system's responsiveness in maintaining a data flow rather than micro-benchmarking individual real-time event latency through the entire plugin pipeline._
_\*Conversion time for TEST-DP01 registered as \<1\* ms. This is attributed to the millisecond precision of the Dart timer; the actual conversion time is likely in the microsecond range but below the timer's resolution for this dataset size._
...
The subsequent analysis is structured to systematically examine the results in relation to each research question. Key areas of focus include the plugin's effectiveness in data normalization, the identified requirements and challenges pertinent to its implementation in a mobile environment, and the suitability of the Open mHealth schema for representing the targeted health data. Throughout this chapter, all interpretations are rigorously grounded in the data presented in Chapter 5 to ensure objectivity and a clear line of reasoning.
== 6.2 Analysis of the Artifacts Core Functionality and Correctness
This analysis section evaluates the 'wearable_health' plugin's core capabilities—specifically, data extraction completeness and the accuracy of its data normalization processes. The interpretations are grounded in the experimental results presented for data retrieval (Section 5.2.1) and conversion accuracy (Section 5.2.2), and seek to provide insights relevant to research questions RQ1 (effective normalization) and RQ3 (Open mHealth effectiveness).
...
The implemented strategies for interoperability, such as abstracting platform-specific permission handling  and data extraction, were positively received by stakeholders (See Apendix A). Feedback indicated that the plugin's ability to handle both sources smoothly and simplify authorization is valuable.

These arguments and evidence highlight both the functional capabilities of the developed 'wearable_health' plugin within its specific operational context and the inherent complexities, limitations, and design trade-offs encountered.
...
Critically reflecting on the project, a broader evaluation scope, encompassing more diverse wearable devices and more rigorously defined real-time scenarios, alongside direct usability testing of the plugin’s API, could have yielded more comprehensive insights. Specifically, a more rigorously detailed methodological account for the verification of the '100% completeness and accuracy' claims in Chapter 2, and the inclusion of quantitative end-to-end latency data for the real-time tests to more fully address RQ2, would have significantly strengthened the study's empirical grounding.
